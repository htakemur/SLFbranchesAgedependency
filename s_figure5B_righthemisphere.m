function s_figure5B_righthemisphere

% Perform multiple linear regression analysis on FA along the right SLF I, by excluding the variance of the FA along the SLF I and II, and then plot the age-dependency curve of residual FA. This script aims to reproduce the right panel of Figure 5B in a following article:
%
...
x(:,2) = transpose(fa_plot(6,:)); % Right SLF III
...
for ik = 1:length(fa_plot)
    predict_r_slf1(ik) = mdl.Coefficients.Estimate(1) + mdl.Coefficients.Estimate(2)*x(ik,1) + mdl.Coefficients.Estimate(3)*x(ik,2);
end
